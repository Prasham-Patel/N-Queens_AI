...
class Node:
    ...
    def get_print_action(self):
        if self.tag is None or self.tag == "START" or self.tag == "HOLE":
            ...
            for j in range(col):
                ...
                if state == "BARRIER":
                    ...
                    if True:
                        ...
                        plt.annotate(self.node_World[i][j].hole, ha='center', va='bottom', xytext=(j, i - 0.1), xy=(j, i - 0.1))
                elif state >= 1:
    ...
    def Q_learning(self):
        ...
        while t_end - t_init <= self.time_to_learn:
            ...
            while first or (not current_node.terminal > 0 and t_end - t_init <= self.time_to_learn):
                ...
                new_pos = self.takeAction([current_node.row, current_node.col],
                                          action)  # the action that agent actually took
