...
class two_link:
    ...
    def get_next_state(self, t, y):
        ...
        theta_ddot_1 = (
                                   I2 * self.torq1 - I2 * self.torq2 + m2 * r2 ** 2 * self.torq1 - m2 * r2 ** 2 * self.torq2 + l1 * m2 ** 2 * r2 ** 3 * theta_dot_1 ** 2 * sin(
                               theta_2) + l1 * m2 ** 2 * r2 ** 3 * theta_dot_2 ** 2 * sin(
                               theta_2) + g * l1 * m2 ** 2 * r2 ** 2 * sin(theta_1) + \
                                   I2 * g * l1 * m2 * sin(theta_1) + I2 * g * m1 * r1 * sin(
                               theta_1) - l1 * m2 * r2 * self.torq2 * cos(theta_2) + \
                                   2 * l1 * m2 ** 2 * r2 ** 3 * theta_dot_1 * theta_dot_2 * sin(theta_2) + \
                                   l1 ** 2 * m2 ** 2 * r2 ** 2 * theta_dot_1 ** 2 * cos(theta_2) * sin(
                               theta_2) - g * l1 * m2 ** 2 * r2 ** 2 * sin(theta_1 + \
                                                                           theta_2) * cos(
                               theta_2) + I2 * l1 * m2 * r2 * theta_dot_1 ** 2 * sin(theta_2) + \
                                   I2 * l1 * m2 * r2 * theta_dot_2 ** 2 * sin(
                               theta_2) + g * m1 * m2 * r1 * r2 ** 2 * sin(theta_1) + \
                                   2 * I2 * l1 * m2 * r2 * theta_dot_1 * theta_dot_2 * sin(theta_2)) / (
                                   - l1 ** 2 * m2 ** 2 * r2 ** 2 * cos(theta_2) ** 2 + \
                                   l1 ** 2 * m2 ** 2 * r2 ** 2 + I2 * l1 ** 2 * m2 + m1 * m2 * r1 ** 2 * r2 ** 2 + I1 * m2 * r2 ** 2 + I2 * m1 * r1 ** 2 + I1 * I2)

        theta_ddot_2 = -(
                    I2 * self.torq1 - I1 * self.torq2 - I2 * self.torq2 - l1 ** 2 * m2 * self.torq2 - m1 * r1 ** 2 * self.torq2 + m2 * r2 ** 2 * self.torq1 - m2 * r2 ** 2 * self.torq2 +
                    l1 * m2 ** 2 * r2 ** 3 * theta_dot_1 ** 2 * sin(
                theta_2) + l1 ** 3 * m2 ** 2 * r2 * theta_dot_1 ** 2 * sin(theta_2) +
                    l1 * m2 ** 2 * r2 ** 3 * theta_dot_2 ** 2 * sin(theta_2) - g * l1 ** 2 * m2 ** 2 * r2 * sin(
                theta_1 + theta_2) -
                    I1 * g * m2 * r2 * sin(theta_1 + theta_2) + g * l1 * m2 ** 2 * r2 ** 2 * sin(
                theta_1) + I2 * g * l1 * m2 * sin(theta_1) +
                    I2 * g * m1 * r1 * sin(theta_1) + l1 * m2 * r2 * self.torq1 * cos(
                theta_2) - 2 * l1 * m2 * r2 * self.torq2 * cos(theta_2) +
                    2 * l1 * m2 ** 2 * r2 ** 3 * theta_dot_1 * theta_dot_2 * sin(theta_2) +
                    2 * l1 ** 2 * m2 ** 2 * r2 ** 2 * theta_dot_1 ** 2 * cos(theta_2) * sin(theta_2) +
                    l1 ** 2 * m2 ** 2 * r2 ** 2 * theta_dot_2 ** 2 * cos(theta_2) * sin(
                theta_2) - g * l1 * m2 ** 2 * r2 ** 2 * sin(theta_1 +
                                                            theta_2) * cos(theta_2) + g * l1 ** 2 * m2 ** 2 * r2 * cos(
                theta_2) * sin(theta_1) -
                    g * m1 * m2 * r1 ** 2 * r2 * sin(theta_1 + theta_2) + I1 * l1 * m2 * r2 * theta_dot_1 ** 2 * sin(
                theta_2) +
                    I2 * l1 * m2 * r2 * theta_dot_1 ** 2 * sin(theta_2) + I2 * l1 * m2 * r2 * theta_dot_2 ** 2 * sin(
                theta_2) +
                    ...
                    2 * I2 * l1 * m2 * r2 * theta_dot_1 * theta_dot_2 * sin(theta_2) +
                    g * l1 * m1 * m2 * r1 * r2 * cos(theta_2) * sin(theta_1)) / (
                                   - l1 ** 2 * m2 ** 2 * r2 ** 2 * cos(theta_2) ** 2 +
                                   l1 ** 2 * m2 ** 2 * r2 ** 2 + I2 * l1 ** 2 * m2 + m1 * m2 * r1 ** 2 * r2 ** 2 + I1 * m2 * r2 ** 2 + I2 * m1 * r1 ** 2 + I1 * I2)
    ...
    def ODE45(self, tf, theta_1, theta_dot_1, theta_2, theta_dot_2):
        ...
        sol = solve_ivp(self.get_next_state, [0, tf], [theta_1, theta_dot_1, theta_2, theta_dot_2], t_eval=t_eval,
